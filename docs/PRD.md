...
> **提交请看 [`docs/PRODUCT_MEMO.md`](PRODUCT_MEMO.md)**（1–2 页）。本文是开发过程记录，部分早期命名与主路径已过时。
>
> 对外产品名：**情报驱动 面试备战**。
> 一句话：先摸清这场怎么考，再对照简历练。
...
- **系统自动获取**：按公司/岗位检索公开面经（牛客、博客、招聘页等；自动排除知乎与小红书）。不登录、不读 Cookie、不绕过验证码。可选接入 Tavily / 博查；未配置时回退公开搜索。
