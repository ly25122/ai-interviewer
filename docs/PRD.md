...
面向大厂技术实习的 **AI 面试官**：把散落在小红书、真假混杂的面经提纯成岗位**考纲**，再变成个人**准备度地图**，并用追问验证掌握是否站得住。
...
| 编号 | 功能 | 说明 |
|---|---|---|
| F0 | **简历 × JD 深挖面试** | 上传简历与岗位 JD，生成攻击计划并多轮追问验证 |
...
1. 用户粘贴 / 上传简历与目标岗位 JD
2. 引擎对照两者产出 4–6 个攻击点：`resume_match`（重合深挖）、`resume_risk`（简历泡沫）、`jd_gap`（JD 有要求但简历弱）
3. 每个攻击点最多两轮追问，只判定是否有新增可验证事实
4. 复盘页给出「经得起 / 经不起」清单
...
- **不做 PDF/Word 复杂解析**：上传以文本为主（.txt/.md），Office/PDF 请粘贴正文——解析链路不稳且拖垮交付
