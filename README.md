...
情报驱动的 AI 面试备战。先摸清这家公司、这个岗位真实怎么考，再对照简历练；不是随机刷题。
...
1. **目标岗位**：填写公司、岗位，粘贴或上传简历与 JD（PDF / DOCX / TXT / MD）。
2. **面试情报**：公开收集（自动检索、链接抓取）或手动加入（整理粘贴、上传文件）。每条可标可信度。条数太少、又对不上岗位时，不会硬画画像。
3. **岗位画像**：把情报聚成高频考点，并对照简历给出匹配情况。
4. **针对训练**：先定时长、题量和难度。完整模拟会覆盖四类题源；情报针对训练只练该组反复出现的点。
...
| 标签 | 含义 |
|---|---|
| 面经考点 | 该岗位或该组面经中出现过，非通用题库抽取 |
| 经历已覆盖 | 职位要求与简历经历重合，将围绕实际贡献追问 |
| 经历待核实 | 量化结果或职责表述偏满，追问中需给出可核验依据 |
| 要求未覆盖 | 职位描述有明确要求，简历中缺少对应经历 |
