...
> 不只是简历 × JD。把你手里的**面试情报**一起喂进来，AI 照这个组的真实考法追问你。

**核心输入 = 简历 × JD × 面试情报。**

**主路径：** 上传简历 + 岗位 JD + 面试情报 → 生成面试提纲（情报命中的点优先追）→ 多轮追问验证你写的经历站不站得住。
...
真正有价值的情报很多**根本不在搜索引擎里**：师兄去年面这个组的口述、微信群里整理的面经、只在某个帖子角落出现的真题。产品把情报当成第一等公民，来源分两条路：

- **系统自动获取**：在模拟面试页填公司/岗位，检索牛客/博客等公开面经（不含知乎、小红书；不登录、不绕验证码）。可选配置 `TAVILY_API_KEY` / `BOCHA_API_KEY`。
- **用户主动提供**（当前主力、也是护城河）：直接粘贴 / 上传 PDF·截图·Markdown·TXT / 给链接系统抓正文。每条可标注来源与可信度（一手 / 公开 / 存疑）。

情报里反复出现、这个组真实考过的点会被标成 `intel_hit`，出题优先级最高——因为那是"内部消息"。
